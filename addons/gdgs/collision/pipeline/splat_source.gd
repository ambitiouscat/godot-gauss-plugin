...
const BYTES_PER_SPLAT := STRUCT_SIZE * BYTES_PER_FLOAT
...
static func snapshot(resource: Object) -> Dictionary:
	...
	for property_name in [&"point_count", &"aabb"]:
	...
	var point_data := _canonical_bytes(resource, point_count)
	...
	if point_data.size() != point_count * BYTES_PER_SPLAT:
		return _failure(
			"Gaussian canonical byte length (%d) does not match point_count × %d (%d)." %
			[point_data.size(), BYTES_PER_SPLAT, point_count * BYTES_PER_SPLAT]
		)
	...
	return {
		...
		"snapshot": {
			...
			"point_data_byte": point_data.duplicate(),
			"aabb": resource_aabb,
		},
	}
...
static func prepare(data_snapshot: Dictionary, control: RefCounted = null) -> Dictionary:
	...
	var point_data: PackedByteArray = data_snapshot.get("point_data_byte", PackedByteArray())
	if point_count <= 0 or point_data.size() != point_count * BYTES_PER_SPLAT:
	...
	for source_index in point_count:
		...
		var base := source_index * BYTES_PER_SPLAT
		var position := Vector3(
			point_data.decode_float(base),
			point_data.decode_float(base + BYTES_PER_FLOAT),
			point_data.decode_float(base + BYTES_PER_FLOAT * 2)
		)
		var covariance_base := base + COVARIANCE_OFFSET * BYTES_PER_FLOAT
		var xx := point_data.decode_float(covariance_base)
		var xy := point_data.decode_float(covariance_base + BYTES_PER_FLOAT)
		var xz := point_data.decode_float(covariance_base + BYTES_PER_FLOAT * 2)
		var yy := point_data.decode_float(covariance_base + BYTES_PER_FLOAT * 3)
		var yz := point_data.decode_float(covariance_base + BYTES_PER_FLOAT * 4)
		var zz := point_data.decode_float(covariance_base + BYTES_PER_FLOAT * 5)
		var opacity := point_data.decode_float(base + OPACITY_OFFSET * BYTES_PER_FLOAT)
...
static func _canonical_bytes(resource: Object, point_count: int) -> PackedByteArray:
	if resource.has_method("get_canonical_point_bytes"):
		var method_value: Variant = resource.call("get_canonical_point_bytes")
		if typeof(method_value) == TYPE_PACKED_BYTE_ARRAY:
			return method_value

	if _has_property(resource, &"point_data_byte"):
		var byte_value: Variant = resource.get("point_data_byte")
		if typeof(byte_value) == TYPE_PACKED_BYTE_ARRAY and byte_value.size() == point_count * BYTES_PER_SPLAT:
			return byte_value

	if _has_property(resource, &"point_data_float"):
		var float_value: Variant = resource.get("point_data_float")
		if typeof(float_value) == TYPE_PACKED_FLOAT32_ARRAY and float_value.size() == point_count * STRUCT_SIZE:
			return float_value.to_byte_array()
