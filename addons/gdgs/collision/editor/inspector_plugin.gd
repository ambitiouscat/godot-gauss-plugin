...
func _on_generate_pressed(object: Object, panel: PanelContainer) -> void:
	...
	var gaussian: Variant = object.call("get_active_gaussian") if object.has_method("get_active_gaussian") else object.get("gaussian")
	var snapshot_result: Dictionary = PIPELINE_SCRIPT.create_snapshot(gaussian)
...
func _looks_like_gaussian_resource(resource: Object) -> bool:
	if resource == null or not is_instance_valid(resource):
		return false
	return (
		_has_property(resource, &"point_count") and _has_property(resource, &"aabb") and
		(_has_property(resource, &"point_data_byte") or _has_property(resource, &"point_data_float"))
	)
