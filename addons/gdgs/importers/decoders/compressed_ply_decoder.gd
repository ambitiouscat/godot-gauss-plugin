...
static func decode(
	...
	if count > max_point_count:
		return DecodeControl.error(ERR_OUT_OF_MEMORY, "Compressed PLY vertex count exceeds the configured limit", "GDGS_DECODE_LIMIT")
	var expected_chunks := int((count + 255) / 256)
	...
	for i in chunks.size():
		if i % check_interval == 0:
			if DecodeControl.is_cancelled(cancellation_callback):
				return DecodeControl.cancellation_error()
			DecodeControl.report(progress_callback, 0.2 + 0.1 * float(i) / float(maxi(chunks.size(), 1)))
		...
		for value in chunk.values():
			if not is_finite(float(value)):
				return DecodeControl.error(ERR_INVALID_DATA, "Compressed PLY contains non-finite chunk metadata", "GDGS_NON_FINITE")
		chunks[i] = chunk
	...
	for i in count:
		...
		if not DecodeControl.finite_vector3(position):
			return DecodeControl.error(ERR_INVALID_DATA, "Compressed PLY contains a non-finite position", "GDGS_NON_FINITE")
		...
		if not DecodeControl.finite_vector3(scale):
			return DecodeControl.error(ERR_INVALID_DATA, "Compressed PLY contains a non-finite scale", "GDGS_NON_FINITE")
