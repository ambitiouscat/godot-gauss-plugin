...
const GPU_TIMING := preload("res://addons/gdgs/runtime/render/gaussian_gpu_timing.gd")
const DIRECT_TEXTURE_SHADER_PATH := "res://addons/gdgs/runtime/debug/shaders/direct_texture_overlay.gdshader"
...
var shader: RID
var pipeline: RID
var paged_shader: RID
var paged_pipeline: RID
...
var _direct_texture_shader: Shader
...
var _compute_shader_initialization_attempts := 0
...
func _notification(what: int) -> void:
	...
	if rd != null:
		...
		if shader.is_valid():
			rd.free_rid(shader)
		if paged_pipeline.is_valid():
			rd.free_rid(paged_pipeline)
		if paged_shader.is_valid():
			rd.free_rid(paged_shader)
...
func _render_callback(_effect_callback_type: int, render_data: RenderData) -> void:
	...
	for view in scene_buffers.get_view_count():
		...
		var gsplat_result: Dictionary = manager.render_for_compositor(
			...
			"compositor-%d-view-%d" % [get_instance_id(), view],
			1
		)
		...
		var gpu_timing_token := int(gsplat_result.get("gpu_timing_token", 0))
		...
		# A source_path node is intentionally payload-free until request_load().
		# Do not touch either compositor GLSL until the render manager has produced
		# actual Gaussian output for this view.  In particular, merely opening a
		# scene containing a CompositorEffect must not trigger shader loading.
		if not _ensure_compute_shader_initialized():
			continue
		...
		var active_pipeline := pipeline
		if (
			color_alpha_format
			== RenderingDevice.DATA_FORMAT_R16G16B16A16_SFLOAT
		):
			active_shader = paged_shader
			active_pipeline = paged_pipeline
		elif (
			color_alpha_format
			!= RenderingDevice.DATA_FORMAT_R32G32B32A32_SFLOAT
		):
		...
		var uniform_set: RID = UniformSetCacheRD.get_cache(active_shader, 0, [
			scene_uniform,
			gsplat_uniform,
		...
		if gpu_timing_token > 0:
			rd.capture_timestamp(GPU_TIMING.marker(gpu_timing_token, "composite_begin"))
		...
		rd.compute_list_end()
		if gpu_timing_token > 0:
			rd.capture_timestamp(GPU_TIMING.marker(gpu_timing_token, "composite_end"))
...
func _ensure_compute_shader_initialized() -> bool:
	if _has_compute_shader_resources():
		return true
	# CompositorEffect callbacks execute on Godot's render thread, which is the
	# only context allowed to create RenderingDevice resources.
	initialize_compute_shader()
	return _has_compute_shader_resources()

func _has_compute_shader_resources() -> bool:
	return (
		rd != null
		and shader.is_valid()
		and pipeline.is_valid()
		and paged_shader.is_valid()
		and paged_pipeline.is_valid()
		and depth_sampler.is_valid()
		and fallback_depth_texture.is_valid()
	)

func initialize_compute_shader() -> void:
	if _has_compute_shader_resources():
		return
	_compute_shader_initialization_attempts += 1
	rd = RenderingServer.get_rendering_device()
	if not rd:
		return
	_free_compute_shader_resources()
	...
	if not pipeline.is_valid():
		_free_compute_shader_resources()
		return
	var paged_glsl_file: RDShaderFile = load(
		"res://addons/gdgs/runtime/compositor/shaders/gaussian_paged_composite.glsl"
	)
	if paged_glsl_file == null:
		_free_compute_shader_resources()
		return
	paged_shader = rd.shader_create_from_spirv(
		paged_glsl_file.get_spirv()
	)
	if not paged_shader.is_valid():
		_free_compute_shader_resources()
		return
	paged_pipeline = rd.compute_pipeline_create(paged_shader)
	if not paged_pipeline.is_valid():
		_free_compute_shader_resources()
		return
	var sampler_state := RDSamplerState.new()
	depth_sampler = rd.sampler_create(sampler_state)
	fallback_depth_texture = _create_fallback_depth_texture()
	if not _has_compute_shader_resources():
		_free_compute_shader_resources()

func _free_compute_shader_resources() -> void:
	if rd == null:
		return
	if fallback_depth_texture.is_valid():
		rd.free_rid(fallback_depth_texture)
...
func _configure_direct_texture_overlay(overlay: MeshInstance3D) -> void:
	...
	var direct_texture_shader := _ensure_direct_texture_shader()
	if direct_texture_shader == null:
		return
...
func _ensure_direct_texture_shader() -> Shader:
	if _direct_texture_shader == null:
		_direct_texture_shader = load(DIRECT_TEXTURE_SHADER_PATH) as Shader
	return _direct_texture_shader
