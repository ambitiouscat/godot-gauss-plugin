...
class DeletionQueue:
	...
	func release_ownership(rid: RID) -> RID:
		var rid_idx := queue.find(rid)
		assert(rid_idx != -1, "RID was not found in deletion queue.")
		return queue.pop_at(rid_idx)
...
func create_empty_storage_buffer(size: int, usage: int = 0) -> Descriptor:
	return Descriptor.new(
		deletion_queue.push(device.storage_buffer_create(size, PackedByteArray(), usage)),
		RenderingDevice.UNIFORM_TYPE_STORAGE_BUFFER
	)

func wrap_borrowed_storage_buffer(rid: RID) -> Descriptor:
	assert(rid.is_valid(), "Borrowed storage buffer RID must be valid.")
	return Descriptor.new(rid, RenderingDevice.UNIFORM_TYPE_STORAGE_BUFFER)

func release_rid_ownership(rid: RID) -> RID:
	return deletion_queue.release_ownership(rid)
