...
void main() {
	...
	if ((range_data.status & RANGE_VALID) == 0u || range_data.requested == 0u) return;
	...
	bool fits = prefix_offset <= telemetry.pair_capacity && range_data.requested <= telemetry.pair_capacity - prefix_offset;
	if (!fits) {
		range_data.admitted_offset = 0xffffffffu;
		atomicAdd(telemetry.dropped_pairs, range_data.requested);
