...
// std430 stride is 48 bytes. Keep this in lock-step with
// GaussianProjectionCapacity.PROJECTION_RANGE_BYTES_PER_SPLAT.
...
layout(std430, set = 0, binding = 3) restrict readonly buffer SplatInstanceIdsBuffer {
	uvec2 splat_instance_data[];
};

layout(std430, set = 0, binding = 4) restrict readonly buffer InstanceTransformsBuffer {
...
ProjectionRange empty_range(in uint status) {
...
vec3 project_covariance(in mat3 covariance_3d, in float scale_modifier, in vec3 mean, in ivec2 image_dims) {
	...
	// Keep centres between the camera and its raster near plane renderable while
	// preventing the Jacobian from dividing by a value arbitrarily close to zero.
...
void store_invalid(in uint id) {
	projection_ranges[id] = empty_range(RANGE_INVALID_PROJECTION);
}

void main() {
	const uint id = gl_GlobalInvocationID.x;
	if (id >= uint(point_count)) return;
	projection_ranges[id] = empty_range(0u);
	...
	// A very large projected Gaussian must be steepened as its extent is capped.
	// Clamping only the rectangle leaves a hard opaque edge and produces the
	// black/grey blocks seen when the camera enters a large anisotropic splat.
	...
	ProjectionRange range_data = empty_range(RANGE_VALID);
