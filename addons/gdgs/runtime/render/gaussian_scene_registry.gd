...
var _resident_page_point_count := 0
var _resident_page_aabb := AABB()
...
func get_resident_page_descriptor() -> Dictionary:
	if _point_data_byte.is_empty() or _resident_page_hash.is_empty():
		return {}
	return {
		"content_identity": "legacy-merged-page",
		"content_hash": _resident_page_hash,
		"schema_version": 1,
		"layout_version": 1,
		"shader_version": 1,
		"point_count": _resident_page_point_count,
		"aabb_min": _resident_page_aabb.position,
		"aabb_max": _resident_page_aabb.end,
		"coordinate_space": "model_local_meters",
		"unaligned_bytes": _point_data_byte.size()
	}
...
func clear() -> Dictionary:
	...
	_resident_page_hash = ""
	_resident_page_point_count = 0
	_resident_page_aabb = AABB()
...
func _sync_scene_resources(force_rebuild: bool) -> Dictionary:
	...
	var resident_page_point_count := 0
	var resident_page_aabb := AABB()
	var has_resident_page_aabb := false
	...
	for node in _splat_nodes:
		...
		if not unique_resources.has(gaussian):
			...
			resident_page_point_count += entry.point_count
			resident_page_aabb = (
				resident_page_aabb.merge(entry.aabb)
				if has_resident_page_aabb
				else entry.aabb
			)
			has_resident_page_aabb = true
		else:
	...
	_resident_page_hash = _sha256_bytes(merged_point_data)
	_resident_page_point_count = resident_page_point_count
	_resident_page_aabb = resident_page_aabb
