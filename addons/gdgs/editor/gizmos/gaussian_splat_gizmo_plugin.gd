...
const MAX_GIZMO_POINTS := 100_000
...
func _redraw(gizmo: EditorNode3DGizmo) -> void:
	...
	if node == null:
		return
	var gaussian: GaussianResource = node.get_active_gaussian() if node.has_method("get_active_gaussian") else node.gaussian
	if gaussian == null:
		return
	var positions: PackedVector3Array = gaussian.extract_positions(MAX_GIZMO_POINTS)
